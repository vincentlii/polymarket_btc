...
class EventQualityValidator:
    ...
    def observe(self, event: TimedMarketEvent) -> DataQualityDecision:
        ...
        if identifier in self._seen_identifiers:
            ...
            return DataQualityDecision(False, self._epoch_id, "duplicate")
        if self._last_available_ts is not None and event.available_ts < self._last_available_ts:
            self._out_of_order_events += 1
            self.mark_gap(reason="available_time_regression")
            return DataQualityDecision(False, self._epoch_id, "out_of_order")
        stale = False
        ...
        self._last_available_ts = event.available_ts
        self._accepted_events += 1
        return DataQualityDecision(True, self._epoch_id, "accepted", stale=stale)
